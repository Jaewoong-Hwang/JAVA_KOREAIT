...
//객체 생성시 한번만 호출되는 메서드
//생성된 객체 내에서는 사용이 불가능
//객체 생성시에 필요한 메모리 공간 형성과 "초기값" 부여에 사용
//생성자 메서드 이름은 클래스 이름과 동일하며 반환자료형은 가지지 않는다.
//
//생성자 함수를 클래스 내에 명시하지 않으면(코드삽입x) 컴파일러에 의해 주입되는 생성자가 있는데 이를 디폴트 생성자라 한다.
...
class C08Simple {
	...
	C08Simple(int n1) { // 반환자료형을 가지지 않기 때문에 void 를 쓰지 않는다
		this.n1 = n1;
...
public class C08ConstructorMain {
	...
	public static void main(String[] args) {
		C08Simple ob0 = new C08Simple();			//deafult 생성자	//디폴트 생성자를 써야하는 이유는..?
