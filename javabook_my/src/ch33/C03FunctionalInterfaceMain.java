...
public class C03FunctionalInterfaceMain {
	...
	// 03
	public static Function<List<Integer>, Integer> func3 = (list) -> list.stream().reduce(0, (sum, el) -> {
		return sum + el;
	});
	// 04
	...
	public static Function<List<Object>, Integer> func4 = (li) -> {
		...
		// 누적함(reduce)
		return li.stream().filter(el -> el instanceof Integer).map(el -> (Integer) el).reduce(0, (sum, el) -> {
			return sum + el;
		});

//		li.stream()
//			.filter((item)->{return item instanceof Integer;})	//Integer형으로 변환
//			.map((item)->{
//				System.out.println(item);
//				return null;
//			})
//			.reduce(0,(sum,item)->{return 0;});
	...
	// 06
	public static Function<Integer, Integer> func6 = func1.andThen(func2); // 함수간 연결이 가능하다. //중첩해서 사용하는 것.
	...
	public static Function<List<Integer>, Integer> subFunc = (x) -> {
		return x.stream().reduce(0, (sum, x1) -> sum + x1);
	};
...
//		System.out.println(func3.apply(Arrays.asList(10,20,30,40)));
		// 04
		System.out.println(func4.apply(Arrays.asList("홍길동", 10, 20, 30, 40, 10.5, true)));
		// 05
		System.out.println(func5.apply(Arrays.asList("홍길동", 10, 20, 30, 40, 10.5, true)));
		// 06
		System.out.println(func6.apply(5)); // func1 의 함수를 먼저 계산==5*5, 그다음 func2 함수 계산 == 25+25
		// 07
		System.out.println(func7.apply(5));
		// 08
		System.out.println(func8.apply(Arrays.asList("홍길동", 10, 20, 30, 40, 10.5, true)));
		// 09
