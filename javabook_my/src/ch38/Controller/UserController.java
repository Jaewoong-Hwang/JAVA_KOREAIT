...
public class UserController implements SubController {
	...
	public Map<String, Object> execute(Map<String, Object> params) {
		...
		case 1: // C - 회원가임
			...
			System.out.println("[No-1 회원가입] : 유요성 검증 확인 : 0" + isOk);
			if (isOk) {
				response.put("status", false);
	...
	private boolean isValid(UserDto userDto) {
		...
		if (userDto.getUserid().matches("^[0-9].*")) {
			System.out.println("[INVALID] userid의 첫 문자로 숫가자 들어올 수 없습니다.");
			response.put("error", "userid의 첫 문자로 숫자가 들어올 수 없습니다.");
			return false;
		}
		// NULL체크 / 데이터(자료) 수준에서 의미있는 데이터가 포함되어져 있는지 여부

		// userid는 첫문자가 숫자인지 여부 -/or 길이가 1글자인지 등등..
